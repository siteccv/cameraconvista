...
- 15/09/2026 (sera): schema "tre momenti" (aperitivo/cena/dopocena) applicato ai meta di home, menu, cocktail-bar, dove-siamo, eventi-privati + testi blocchi (intro cocktail-bar e eventi-privati, CTA preventivo ora renderizzata in `eventi-privati.tsx`). Vincolo testi: cucina 18.00–22.30, locale 17.30–01.00; "rooftop"/"terrazza" vietati (in EN "dehors/outdoor tables"). `eventi-privati-cena` rimessa `is_visible=false` (noindex + fuori sitemap) finché la card Cena resta spenta — evita soft-404 da redirect client. NOTA blocchi: il sito pubblico legge `published_snapshot` (specchiato in `metadata.__publishedSnapshot`), non le colonne draft — un update via REST deve aggiornare anche lo snapshot.

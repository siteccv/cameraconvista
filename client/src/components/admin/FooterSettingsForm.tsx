...
export function FooterSettingsForm() {
    ...
    if (footerSettings) {
      ...
      const migratedHours = footerSettings.hours.map((entry) => {
        ...
        if ("dayKeyIt" in entry && !("selectedDays" in entry)) {
          const oldEntry = entry as unknown as {
            dayKeyIt: string;
            dayKeyEn: string;
            hours: string;
            isClosed: boolean;
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: t(
          "Le impostazioni del footer sono state salvate.",
          "Footer settings have been saved.",
        ),
      });
    },
    onError: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <Accordion
        type="multiple"
        defaultValue={["about", "contacts", "hours", "social", "legal"]}
        className="space-y-2"
      >
          ...
          <AccordionContent className="space-y-4 pb-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <TranslateButton textIt={formData.about.it} onTranslated={handleTranslateAbout} />
              </div>
              <Textarea
          ...
          <AccordionTrigger className="hover:no-underline py-3">
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium">
                {t("Orari", "Hours")} ({formData.hours.length})
              </span>
            </div>
          </AccordionTrigger>
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            toggleDaySelection(index, day.index, !!checked)
                          }
                        ...
                        <div className="flex items-center gap-2 flex-1">
                          ...
                          >
                            <SelectTrigger
                              ...
                              data-testid={`select-hours-open-${index}`}
                            >
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                          ...
                          >
                            <SelectTrigger
                              className="w-24 h-9 text-sm"
                              data-testid={`select-hours-close-${index}`}
                            >
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
        ...
        <AccordionItem value="social" className="border rounded-lg px-4">
          <AccordionTrigger className="hover:no-underline py-3">
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium">
                {t("Social", "Social")} ({formData.social.length})
              </span>
            </div>
          </AccordionTrigger>
          <AccordionContent className="space-y-3 pb-4">
            {formData.social.map((link, index) => (
              <div key={index} className="flex items-center gap-2">
                <Select value={link.type} onValueChange={(val) => updateSocial(index, "type", val)}>
                  <SelectTrigger
                    className="w-32 h-9 text-sm"
                    data-testid={`select-social-type-${index}`}
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>

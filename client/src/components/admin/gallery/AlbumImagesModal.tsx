...
export function AlbumImagesModal({ open, onClose, gallery }: AlbumImagesModalProps) {
  ...
  const { data: serverImages = [], isLoading } = useQuery<GalleryImage[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/admin/galleries/${gallery.id}/images`, {
        credentials: "include",
      });
  ...
  const updateImageMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<InsertGalleryImage> }) => {
      const response = await apiRequest(
        "PATCH",
        `/api/admin/galleries/${gallery.id}/images/${id}`,
        data,
      );
      return response.json();
    },
    ...
    onError: () => {
      toast({
        ...
        description: t("Impossibile aggiornare l'immagine.", "Failed to update image."),
        variant: "destructive",
      });
    },
  });
  ...
  const reorderMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: t("Errore", "Error"),
        description: t("Impossibile riordinare le immagini.", "Failed to reorder images."),
        variant: "destructive",
      });
  ...
  const deleteImageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/galleries", gallery.id, "images"] });
      toast({
        title: t("Eliminata", "Deleted"),
        description: t("Immagine rimossa dall'album.", "Image removed from album."),
      });
    },
    onError: () => {
      toast({
  ...
  const handleMediaSelect = async (mediaItems: Media[]) => {
    ...
    let nextSortOrder =
      orderedImages.length > 0 ? Math.max(...orderedImages.map((img) => img.sortOrder)) + 1 : 0;
    ...
    for (const media of mediaItems) {
      try {
        ...
        toast({
          title: t("Errore", "Error"),
          description: t(
            `Impossibile aggiungere "${media.filename}".`,
            `Failed to add "${media.filename}".`,
          ),
          variant: "destructive",
        });
      }
    ...
    if (addedCount > 0) {
      toast({
        ...
        description:
          addedCount > 1
            ? t(
                `${addedCount} immagini aggiunte all'album.`,
                `${addedCount} images added to album.`,
              )
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm mt-2">
                  {t("Clicca 'Aggiungi Immagine' per iniziare.", "Click 'Add Image' to start.")}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
